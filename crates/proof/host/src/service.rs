...
impl<B: ProverBackend> ProverService<B> {
    ...
    async fn prove_block_inner(
        ...
        let mut witness_timer = base_metrics::timed!(Metrics::witness_build_duration_seconds());
        let oracle = host.build_witness(oracle).await.map_err(ProverError::Host)?;
        witness_timer.stop();

        let mut prover_timer = base_metrics::timed!(Metrics::prover_duration_seconds());
